#!/usr/bin/env bash
# 模式 B 仿真：机载端随机扰动 + 虚拟主臂（QP-WBC + 积分动作）
set -eo pipefail
...
pkill -f "master_arm_node|teleop_wbc|teleop_wbc.launch|master_motion_demo|mount_disturbance_profile" 2>/dev/null || true
sleep 0.5

cat <<EOF

╔══════════════════════════════════════════════════════════╗
║  模式 B 仿真：机载端扰动 + QP-WBC + 积分动作              ║
╚══════════════════════════════════════════════════════════╝
...
ros2 launch "${PROJECT_DIR}/launch/teleop_wbc.launch.py" \
  ...
  teleop_config:=teleop_wbc_disturbed.yaml \
  use_rviz:=True &
LAUNCH_PID=$!
sleep 3
